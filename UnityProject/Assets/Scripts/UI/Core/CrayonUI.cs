...
namespace UI.Core
{
	public class CrayonUI : MonoBehaviour
	{
		...
		private void OnTileSelect(int newCategoryIndex, int newIndex)
		{
			...
			LocalPlayerManager.CurrentMind.playerNetworkActions.CmdSetCrayon(openingObject, (uint)categoryIndex, (uint)index, (uint)colourDropDown.value, (OrientationEnum)directionDropDown.value);
			gameObject.SetActive(false);
		}
