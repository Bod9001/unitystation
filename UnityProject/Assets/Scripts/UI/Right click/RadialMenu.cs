...
{
	/// <summary>
	/// To indicate the submenu was blocked by shift so It can be opened up when shift is released
	/// </summary>
	public bool BlockSelection = false;

	/// <summary>
	/// To override Forcing redraw a selection menu
	/// </summary>
	public bool OverrideSelection = false;
	...
	void Update()
	{
		if (initialized)
		{
			...
			if (BlockSelection && !Input.GetKey(KeyCode.LeftShift)) {
				Logger.Log("ff");
				BlockSelection = false;
				OverrideSelection = true;
				ButtonLayer selectedLayer = buttonLayers[0];
				SelectButton(selectedLayer, selectedLayer.SelectedButton);
			}
		}
	...
	private void SelectButton(ButtonLayer layer, RadialButton button)
	{
		...
		if (layer.SelectedButton != button || OverrideSelection )
		{
			...
			if (button.HasSubMenus() && !Input.GetKey(KeyCode.LeftShift))
			{
				SpawnButtons(button.MenuItem.SubMenus, button.MenuDepth + 1, button.GlobalAngle);
			}
			if (Input.GetKey(KeyCode.LeftShift))
			{
				Logger.Log("BlockSelection = tr");
				BlockSelection = true;
			}
