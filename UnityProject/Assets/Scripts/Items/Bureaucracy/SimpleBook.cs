...
public class SimpleBook : MonoBehaviour, ICheckedInteractable<HandActivate>
{
	...
	[Tooltip("How long each page (or remark) takes to read.")] [BoxGroup("Settings"), SerializeField]
	private float timeToReadPage = 5f;

	[Tooltip("Whether this book can be read by the same person multiple times.")] [BoxGroup("Settings"), SerializeField]
	...
	{
		var playerTile = player.registerTile;
		...
		void ReadPage()
		{
			...
			SoundManager.PlayNetworkedAtPos(pageturnSfx.PickRandom(), playerTile.WorldPositionServer, sourceObj: player.GameObjectBody);
			Chat.AddExamineMsgFromServer(player, remarks.PickRandom());
