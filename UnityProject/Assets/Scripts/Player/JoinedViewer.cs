...
public class JoinedViewer : NetworkBehaviour
{
		...
		string unverifiedToken)
	{
		...
		if (RelatedConnectedPlayer == null)
		{
			...
			RelatedConnectedPlayer = PlayersManager.Instance.GetNew(connectionToClient, this, unverifiedUsername,
				unverifiedClientId, Userid);
			PlayersManager.Instance.Add(RelatedConnectedPlayer);
			RelatedConnectedPlayer.TargetUpdateClient(RelatedConnectedPlayer.Connection);
			TargetLocalPlayerSetupNewPlayer(connectionToClient, GameManager.Instance.CurrentRoundState);
		}
		else
		{
			RelatedConnectedPlayer.Connection = connectionToClient;
			RelatedConnectedPlayer.ViewerScript = this;
			PlayersManager.Instance.Add(RelatedConnectedPlayer);
			RelatedConnectedPlayer.TargetUpdateClient(RelatedConnectedPlayer.Connection);
			StartCoroutine(WaitForLoggedOffObserver(RelatedConnectedPlayer, RelatedConnectedPlayer.CurrentMind));
		}
	...
	/// </summary>
	private IEnumerator WaitForLoggedOffObserver(ConnectedPlayer ConnectedPlayer, Mind loggedOffPlayer)
	{
		...
		if (netIdentity != null)
		{
			yield return WaitFor.EndOfFrame;
			TargetLocalPlayerRejoinUI(connectionToClient);
			PlayerSpawn.ServerAssignMind(ConnectedPlayer,loggedOffPlayer);
		}
		else
		{
			Logger.LogError($"No {nameof(NetworkIdentity)} component on {loggedOffPlayer}! " +
