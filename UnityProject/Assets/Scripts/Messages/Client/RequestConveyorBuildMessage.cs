...
namespace Messages.Client
{
	...
	public class RequestConveyorBuildMessage : ClientMessage<RequestConveyorBuildMessage.NetMessage>
	{
		...
		public override void Process(NetMessage msg)
		{
			...
			{
				var spawnedObj = entry.ServerBuild(SpawnDestination.At(SentByPlayer.CurrentMind.registerTile), hasConstructionMenu);
				if (spawnedObj)
				{
					...
					Chat.AddActionMsgToChat(SentByPlayer.CurrentMind, $"You finish building the {entry.Name}.",
						$"{SentByPlayer.CurrentMind.ExpensiveName()} finishes building the {entry.Name}.");
				}
			}
			...
			ToolUtils.ServerUseTool(SentByPlayer.CurrentMind, usedSlot.ItemObject,
				ActionTarget.Tile(SentByPlayer.CurrentMind.registerTile.WorldPositionServer), entry.BuildTime,
				ProgressComplete);
		}
