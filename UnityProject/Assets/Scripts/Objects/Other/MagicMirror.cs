...
namespace Objects
{
	...
	public class MagicMirror : MonoBehaviour
	{
		...
		{
			Mind player = GetPlayer();

			if (newName.Length < 3)
			{
				Chat.AddExamineMsgFromServer(player, "That doesn't seem to be an identifiable name. Too short, perhaps?");
				return;
			}
		...
		private IEnumerator RunNameSetSequence(Mind player, string newName)
		{
			...
			player.OriginalCharacter.SetPermanentName(newName);
			SpawnPaper(player);
		}
