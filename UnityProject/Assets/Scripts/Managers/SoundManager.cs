...
public class SoundManager : MonoBehaviour
{
	...
	private void PlaySource(SoundSpawn source, bool polyphonic = false, bool Global = true, bool forceMixer = false)
	{
		if (!forceMixer)
		{
			...
			{
				if ((PlayerManager.LocalPlayer.TileWorldPosition().To3Int() - source.transform.position).magnitude < 40
				    && MatrixManager.Linecast((Vector3) PlayerManager.LocalPlayer.TileWorldPosition().To3Int(),
