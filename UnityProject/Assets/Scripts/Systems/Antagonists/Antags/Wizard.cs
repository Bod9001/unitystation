...
namespace Antagonists
{
	...
	public class Wizard : Antagonist
	{
		...
		public override void AfterSpawn(Mind player)
		{
			...
			{
				player.OriginalCharacter.SetPermanentName(GetRandomWizardName());
			}
		...
		private void GiveRandomSpells(Mind player)
		{
			...
			Chat.AddExamineMsgFromServer(player, playerMsg.ToString());
		}

		private void SetPapers(Mind player)
		{
			IEnumerable<ItemSlot> idSlots = player.DynamicItemStorage.GetNamedItemSlots(NamedSlot.id);
			...
			IEnumerable<ItemSlot> storage02s = player.DynamicItemStorage.GetPocketsSlots();
