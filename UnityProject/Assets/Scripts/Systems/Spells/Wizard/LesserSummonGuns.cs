...
namespace Systems.Spells.Wizard
{
	...
	public class LesserSummonGuns : Spell
	{
		...
		{
			StartCoroutine(SpawnPortals(caster.BodyWorldPositionInt));
		...
		private bool SpawnGunInHand(Mind caster)
		{
			SpawnResult result = Spawn.ServerPrefab(gunPrefab, caster.BodyWorldPositionInt);
			if (result.Successful)
			{
				GameObject gun = result.GameObject;
				ItemSlot bestSlot = caster.DynamicItemStorage.GetBestHandOrSlotFor(gun);
